 ...
 */
declare function mappedFind<Item, ReturnType>(array: Item[], callback: (item: Item, index: number) => Nil | ReturnType): ReturnType | undefined;
 ...
 */
interface T9NStrings {
    [key: string]: T9NStrings | string;
}
/**
 * The list of supported locales for ArcGIS Maps SDK for JavaScript components.
 */
declare const supportedLocales: Set<string>;
 ...
 * ```
 * @template Strings The type of the T9N strings bundle.
 ...
 * @returns The T9N strings bundle for the given locale.
 */
declare function fetchT9NStringsBundle<Strings extends T9NStrings>(locale: string, assetsPath: string, prefix?: string): Promise<Strings>;
/**
...
type LocaleObserver<Strings extends T9NStrings = T9NStrings> = {
 ...
 * @param component The component to observe for ancestry lang attribute changes.
 * @param assetsPath The path to the assets folder where the T9N strings are located.
 * @param callback The callback to call when the locale changes.
 * @param assetName Optional override the asset file name
 * @returns A promise that resolves when the locale is set.
 */
declare function startLocaleObserver<Strings extends T9NStrings = T9NStrings>(element: HTMLElement, assetsPath: string, onUpdated: (payload: LocaleObserver<Strings>) => void, assetName?: string): IHandle;
...
interface ExportsForTests {
    extractMinorVersion: typeof extractMinorVersion;
}
declare let exportsForTests: ExportsForTests | undefined;

export { Deferred, type IHandle, type LocaleObserver, type Nil, type T9NStrings, addLTRMark, camelToHuman, camelToKebab, capitalize, classes, createFilterExpression, debounce, defaultLocale, devToolsAwareTimeout, exportsForTests, fetchT9NStringsBundle, generateGuid, getElementAttribute, getElementDir, getElementLocales, getElementTheme, getPreamble, hasSameOrigin, isNotNull, isNotUndefined, isURL, kebabToPascal, mappedFind, observeAncestorsMutation, quoteString, safeAsyncCall, safeCall, setFocusOnElement, setValuesInString, startLocaleObserver, supportedLocales };
