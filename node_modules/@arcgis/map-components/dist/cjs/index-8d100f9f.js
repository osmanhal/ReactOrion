...
const index = require('./index-c3c4ddd9.js');
...
var _Controller = class _Controller {
  ...
  triggerDisconnected() {
    ...
    this._lifecycleDisconnected.forEach(index.safeCall);
    this._lifecycleDisconnected = [];
  }
  async triggerLoad() {
  ...
  triggerUpdate() {
    ...
    if (genericController.hostUpdate) {
      index.safeCall(genericController.hostUpdate, genericController);
    }
    this._callbacks.hostUpdate.forEach(index.safeCall);
  }
  triggerUpdated() {
    const genericController = this;
    if (genericController.hostUpdated) {
      index.safeCall(genericController.hostUpdated, genericController);
    }
    this._callbacks.hostUpdated.forEach(index.safeCall);
  }
  triggerDestroy() {
    const genericController = this;
  ...
  constructor(component) {
    ...
    this.trackPropKey = (onResolved, defaultValue) => {
      ...
      this._keyTrackers.push(
        (key, value, previousValue) => index.safeCall(onResolved, void 0, defaultValue === value ? key : void 0, previousValue)
      );
      return defaultValue;
    };
...
var ControllerManager = class extends GenericController {
        ...
        if (this.isLit) {
          ...
          if (details && details?.attribute !== false) {
            component.el.setAttribute(
              typeof details.attribute === "string" ? details.attribute : index.camelToKebab(property),
  ...
  _connectedCallback() {
    ...
    this._controllers.forEach(
      (controller) => "triggerConnected" in controller ? controller.triggerConnected() : index.safeCall(controller.hostConnected, controller)
    );
  ...
  _disconnectedCallback() {
    ...
    this._controllers.forEach(
      (controller) => "triggerDisconnected" in controller ? controller.triggerDisconnected() : index.safeCall(controller.hostDisconnected, controller)
    );
  ...
  async _load() {
    ...
    await Promise.allSettled(
      Array.from(
        this._controllers,
        async (controller) => integrations.has(controller) ? void 0 : "triggerLoad" in controller ? await controller.triggerLoad() : await index.safeAsyncCall(controller.hostLoad, controller)
      )
    );
  ...
  _loaded() {
    this._controllers.forEach(
      (controller) => "triggerLoaded" in controller ? controller.triggerLoaded() : index.safeCall(controller.hostLoaded, controller)
    );
    this._originalLifecycles.componentDidLoad?.call(this.component);
  }
  _update() {
    this._controllers.forEach(
      (controller) => "triggerUpdate" in controller ? controller.triggerUpdate() : index.safeCall(controller.hostUpdate, controller)
    );
    void this._originalLifecycles.componentWillUpdate?.call(this.component);
  }
  _updated() {
    this._controllers.forEach(
      (controller) => "triggerUpdated" in controller ? controller.triggerUpdated() : index.safeCall(controller.hostUpdated, controller)
    );
  ...
  async destroy() {
    ...
    this.destroyed = true;
    this._controllers.forEach(
      (controller) => "triggerDestroy" in controller ? controller.triggerDestroy() : index.safeCall(controller.hostDestroy, controller)
    );
...
var AccessorController = class extends GenericController {
  ...
  async hostLoad() {
    ...
    if (component.reactiveUtils === void 0) {
      this.reactiveUtils = await index.importCoreReactiveUtils();
    } else {
