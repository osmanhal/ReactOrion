...
var import_testing = require("@stencil/core/testing");
...
function trackPropertyKey(object, onResolved, defaultValue) {
  ...
  pendingTrackers.callbacks.push((keys2) => {
    const callback = (key2) => (0, import_components_utils2.safeCall)(onResolved, null, key2);
  ...
  constructor(component) {
    ...
    this.trackPropKey = (onResolved, defaultValue) => {
      ...
      this._keyTrackers.push(
        (key, value, previousValue) => (0, import_components_utils3.safeCall)(onResolved, void 0, defaultValue === value ? key : void 0, previousValue)
      );
      return defaultValue;
    };
...
var useControllerManager = (component, forceUpdate2) => new ControllerManager(component, forceUpdate2);
var ControllerManager = class extends GenericController {
  constructor(component, forceUpdate2) {
    if (process.env.NODE_ENV !== "production") {
    ...
    if (!isLit) {
      const update = forceUpdate2 ?? component.constructor.__forceUpdate;
    ...
    this._updatePromise = new import_components_utils4.Deferred();
        ...
        if (this.isLit) {
          ...
          if (details && details?.attribute !== false) {
            component.el.setAttribute(
              typeof details.attribute === "string" ? details.attribute : (0, import_components_utils4.camelToKebab)(property),
  ...
  _connectedCallback() {
    ...
    this._controllers.forEach(
      (controller) => "triggerConnected" in controller ? controller.triggerConnected() : (0, import_components_utils4.safeCall)(controller.hostConnected, controller)
    );
  ...
  _disconnectedCallback() {
    ...
    this._controllers.forEach(
      (controller) => "triggerDisconnected" in controller ? controller.triggerDisconnected() : (0, import_components_utils4.safeCall)(controller.hostDisconnected, controller)
    );
  ...
  async _load() {
    ...
    if (integrations.size > 0) {
      for (const controller of integrations) {
        if ("triggerLoad" in controller) {
          ...
          await (0, import_components_utils4.safeAsyncCall)(controller.hostLoad, controller);
        }
      }
    }
    await Promise.allSettled(
      Array.from(
        this._controllers,
        async (controller) => integrations.has(controller) ? void 0 : "triggerLoad" in controller ? await controller.triggerLoad() : await (0, import_components_utils4.safeAsyncCall)(controller.hostLoad, controller)
      )
    );
  ...
  _loaded() {
    this._controllers.forEach(
      (controller) => "triggerLoaded" in controller ? controller.triggerLoaded() : (0, import_components_utils4.safeCall)(controller.hostLoaded, controller)
    );
    this._originalLifecycles.componentDidLoad?.call(this.component);
  }
  _update() {
    this._controllers.forEach(
      (controller) => "triggerUpdate" in controller ? controller.triggerUpdate() : (0, import_components_utils4.safeCall)(controller.hostUpdate, controller)
    );
    void this._originalLifecycles.componentWillUpdate?.call(this.component);
  }
  _updated() {
    this._controllers.forEach(
      (controller) => "triggerUpdated" in controller ? controller.triggerUpdated() : (0, import_components_utils4.safeCall)(controller.hostUpdated, controller)
    );
  ...
  async destroy() {
    ...
    this.destroyed = true;
    this._controllers.forEach(
      (controller) => "triggerDestroy" in controller ? controller.triggerDestroy() : (0, import_components_utils4.safeCall)(controller.hostDestroy, controller)
    );
  ...
  _autoDestroyDisabledWatcher() {
    if (!this.component.el.isConnected && !this.component.autoDestroyDisabled) {
      ...
      if (autoDestroyOnDisconnectTimeout === 0) {
        ...
        this._autoDestroyTimeout = (0, import_components_utils5.devToolsAwareTimeout)(destroy, autoDestroyOnDisconnectTimeout);
      }
    }
  }
...
// src/testing/testingUtils.ts
var import_core = require("@stencil/core");
async function spec(component, opts) {
  const additionalComponents = opts?.components ?? [];
  const page = await (0, import_testing.newSpecPage)({
...
function wrapController(controller, afterConstruct) {
  let Test = class {
    constructor() {
      this.manager = useControllerManager(this, import_core.forceUpdate);
  ...
  __decorateClass([
    (0, import_core.Element)()
  ], Test.prototype, "el", 2);
  Test = __decorateClass([
    (0, import_core.Component)({ tag: "te-st" })
  ], Test);
  return Test;
}
