export declare const CSS: {
    wrapper: string;
    wrapperBordered: string;
    container: string;
    containerHover: string;
    containerBorder: string;
    containerBorderSelected: string;
    containerBorderUnselected: string;
    contentContainer: string;
    contentContainerSelectable: string;
    contentContainerHasCenterContent: string;
    nestedContainer: string;
    nestedContainerOpen: string;
    content: string;
    customContent: string;
    actionsStart: string;
    contentStart: string;
    label: string;
    description: string;
    contentEnd: string;
    contentBottom: string;
    actionsEnd: string;
    selectionContainer: string;
    selectionContainerSingle: string;
    openContainer: string;
    dragContainer: string;
    close: string;
};
export declare const SLOTS: {
    actionsStart: string;
    contentStart: string;
    content: string;
    contentBottom: string;
    contentEnd: string;
    actionsEnd: string;
};
export declare const MAX_COLUMNS = 0;
export declare const ICONS: {
    readonly selectedMultiple: "check-square-f";
    readonly selectedSingle: "bullet-point-large";
    readonly unselectedMultiple: "square";
    readonly unselectedSingle: "bullet-point-large";
    readonly closedLTR: "chevron-right";
    readonly closedRTL: "chevron-left";
    readonly open: "chevron-down";
    readonly blank: "blank";
    readonly close: "x";
};
export declare const activeCellTestAttribute = "data-test-active";
