...
import { createTimeZoneItems, findTimeZoneItemByProp, getMessageOrKeyFallback, getSelectedRegionTimeZoneLabel, getUserTimeZoneName, getUserTimeZoneOffset, } from "./utils";
export class InputTimeZone {
    ...
    renderRegionItems() {
        return this.timeZoneItems.flatMap(({ label, items }) => (h("calcite-combobox-item-group", { key: label, label: getMessageOrKeyFallback(this.messages, label) }, items.map((item) => {
            const selected = this.selectedTimeZoneItem === item;
            const { label, value } = item;
            return (h("calcite-combobox-item", { "data-value": value, description: getMessageOrKeyFallback(this.messages, item.metadata.country), key: label, metadata: item.metadata, selected: selected, textLabel: label, value: `${item.filterValue}` }, h("span", { class: CSS.offset, slot: "content-end" }, item.metadata.offset)));
        }))));
    }
    ...
    static get properties() {
        return {
            ...
            "offsetStyle": {
                ...
                "mutable": false,
                "complexType": {
                    "original": "OffsetStyle",
                    "resolved": "\"gmt\" | \"user\" | \"utc\"",
                    "references": {
                        "OffsetStyle": {
                            "location": "import",
                            "path": "./interfaces",
                            "id": "src/components/input-time-zone/interfaces.d.ts::OffsetStyle"
                        }
                    }
                },
                "required": false,
