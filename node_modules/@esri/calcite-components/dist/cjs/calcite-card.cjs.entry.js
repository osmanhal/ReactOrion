...
const Card = class {
    constructor(hostRef) {
        ...
        this.calciteInternalCardKeyEvent = index.createEvent(this, "calciteInternalCardKeyEvent", 6);
        //--------------------------------------------------------------------------
        //
        //  Private Methods
        //
        ...
        this.handleDefaultSlotChange = (event) => {
            this.hasContent = dom.slotChangeHasAssignedElement(event);
        };
        this.keyDownHandler = (event) => {
            if (event.target === this.containerEl && !this.selectable && !this.disabled) {
                if (key.isActivationKey(event.key) && this.selectionMode !== "none") {
    ...
    renderCheckboxDeprecated() {
        return (index.h("calcite-label", { class: CSS.checkboxWrapperDeprecated }, index.h("calcite-checkbox", { checked: this.selected, label: this.messages.select, onClick: this.selectCardDeprecated, onKeyDown: this.cardSelectKeyDownDeprecated })));
    }
    renderThumbnail() {
    ...
    renderSelectionIcon() {
        ...
        return (index.h("div", { class: CSS.checkboxWrapper, onPointerDown: this.cardSelectClick, tabIndex: -1 }, index.h("calcite-icon", { icon: icon, scale: "s" })));
    }
    renderHeader() {
        const { el } = this;
        const heading = dom.getSlotted(el, SLOTS.heading);
        const description = dom.getSlotted(el, SLOTS.description);
        ...
        return hasHeader || hasDeprecatedHeader ? (index.h("header", { class: CSS.header }, this.selectable ? this.renderCheckboxDeprecated() : null, index.h("div", { class: CSS.headerTextContainer }, index.h("slot", { key: "heading-slot", name: SLOTS.heading }), index.h("slot", { key: "description-slot", name: SLOTS.description }), index.h("slot", { key: "deprecated-title-slot", name: SLOTS.title }), index.h("slot", { key: "deprecated-subtitle-slot", name: SLOTS.subtitle })), this.selectionMode !== "none" && this.renderSelectionIcon())) : null;
    }
    renderFooter() {
        const { el } = this;
    ...
    render() {
        ...
        const role = this.selectionMode === "multiple"
            ...
            : this.selectionMode !== "none"
                ? "radio"
                : undefined;
        return (index.h(index.Host, { key: 'e33b76130cd91d3caa6662f1f58b79d238c708fb' }, index.h(interactive.InteractiveContainer, { key: '772da2a3a1274eb467d1febb915ffb97953beae7', disabled: this.disabled }, index.h("div", { key: 'b9fdbb51cfb07964479d0f723add39b8a2384f86', "aria-checked": this.selectionMode !== "none" ? dom.toAriaBoolean(this.selected) : undefined, "aria-disabled": this.disabled, "aria-label": this.label, class: { [CSS.contentWrapper]: true, inline: thumbnailInline }, onClick: this.cardBodyClickHandler, onKeyDown: this.keyDownHandler, ref: (el) => (this.containerEl = el), role: role, tabIndex: !this.selectable || this.disabled ? 0 : -1 }, this.loading ? (index.h("div", { "aria-live": "polite", class: "calcite-card-loader-container" }, index.h("calcite-loader", { label: this.messages.loading }))) : null, thumbnailStart && this.renderThumbnail(), index.h("section", { key: '51cfe132a589ec65b39544b4c1246545b0b864f2', "aria-busy": dom.toAriaBoolean(this.loading), class: { [CSS.container]: true } }, this.renderHeader(), index.h("div", { key: 'c256998e330a949d47a12b23127c68cba081854e', class: {
                [CSS.cardContent]: true,
                [CSS.hasSlottedContent]: this.hasContent,
            } }, index.h("slot", { key: '17fb9b0a9919f09a474d3c54818e770be9a85b77', onSlotchange: this.handleDefaultSlotChange })), this.renderFooter()), !thumbnailStart && this.renderThumbnail()))));
    }
